...
var _ = Describe("helmer_InstallCRDs", func() {
	...
	It("should return an error when a CRD cannot be created", func() {
		...
		mockResourceAPI.
			EXPECT().
			CreateFromYAML(context.TODO(), nil, false, owner, name, namespace, nil, "", "", nil).
			Return(randomError)
...
`)
		...
		mockResourceAPI.
			EXPECT().
			CreateFromYAML(context.TODO(), manifests, false, owner, name, namespace, nil, "", "", nil)
...
var _ = Describe("helmer_Run", func() {
	...
	It("should fail with an unsupported chart type", func() {
		...
		h, err := newHelmerWithVersions(mockResourceAPI, cli.New(), nil, nil, mockKubeClient)
		Expect(err).NotTo(HaveOccurred())
		err = h.Run(context.TODO(), ch, nil, owner, name, namespace, nil, "", "", nil, false)
	...
	It("should fail if CRD installation fails", func() {
		...
		mockResourceAPI.
			EXPECT().
			CreateFromYAML(context.TODO(), gomock.Any(), false, owner, name, namespace, nil, "", "", nil).
			Return(randomError)
		h, err := newHelmerWithVersions(mockResourceAPI, cli.New(), nil, nil, mockKubeClient)
		Expect(err).NotTo(HaveOccurred())
		err = h.Run(context.TODO(), ch, nil, owner, name, namespace, nil, "", "", nil, false)
