...
type StateDriverValidationDecoded struct {
	pod corev1.Pod
}
...
var stateDriverValidationDecoded StateDriverValidationDecoded
...
func DecodeStateDriverValidation() {
	s := json.NewYAMLSerializer(json.DefaultMetaFactory, scheme.Scheme,
		scheme.Scheme)
	_, _, err := s.Decode(stateDriverValidationManifests.pod, nil, &stateDriverValidationDecoded.pod)
	if err != nil {
		panic(err)
	}
